...
{
    return (u ^ v) * 0x010001A7 ;
}

inline hash512 fnv1a(const hash512& u, const hash512& v) noexcept
{
    hash512 r;
    for (size_t i = 0; i < sizeof(r) / sizeof(r.half_words[0]); ++i)
        r.half_words[i] = fnv1a(u.half_words[i], v.half_words[i]);
    return r;
}
...
hash1024 calculate_dataset_item(const epoch_context& context, uint32_t index) noexcept
{
    ...
    {
        uint32_t t0 = fnv1a(init0 ^ j, mix0.half_words[j % num_half_words]);
        int64_t parent_index0 = t0 % num_cache_items;
        mix0 = fnv1a(mix0, fix_endianness32(cache[parent_index0]));

        uint32_t t1 = fnv1a(init1 ^ j, mix1.half_words[j % num_half_words]);
        int64_t parent_index1 = t1 % num_cache_items;
        mix1 = fnv1a(mix1, fix_endianness32(cache[parent_index1]));
    }
    ...
    const epoch_context& context, const hash512& seed, lookup_fn lookup) noexcept
{
    ...
    {
        const uint32_t p = fnv1a(i ^ seed_init, mix.hwords[i % mix_hwords]) % index_limit;
        const hash1024 newdata = fix_endianness32(lookup(context, p));

        for (size_t j = 0; j < mix_hwords; ++j)
            mix.hwords[j] = fnv1a(mix.hwords[j], newdata.hwords[j]);
    }

    hash256 mix_hash;
    for (size_t i = 0; i < mix_hwords; i += 4)
    {
        const uint32_t h1 = fnv1a(mix.hwords[i], mix.hwords[i + 1]);
        const uint32_t h2 = fnv1a(h1, mix.hwords[i + 2]);
        const uint32_t h3 = fnv1a(h2, mix.hwords[i + 3]);
